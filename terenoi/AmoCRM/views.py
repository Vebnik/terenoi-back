    ...
    def post(self, request):
        if self.request.data:
            ...
            if res.get('leads[update][0][id]'):
                ...
                if lead:
                    data = get_leads_data(res, amo_token, lead_id)
...
class WebHooksCustomers(APIView):
        ...
        if self.request.data:
            ...
            if res.get('customers[update][0][id]'):
                ...
                if customer:
                    ...
                    data = get_customer_data(res, amo_token, customer_id)
                    ...
                    if res.get('customers[update][0][next_date]'):
                        next_date = datetime.datetime.utcfromtimestamp(
                            int(res.get('customers[update][0][next_date]'))).strftime(
                            '%Y-%m-%d %H:%M:%S')
                ...
                if not customer:
                    ...
                    data = get_customer_data(res, amo_token, customer_id)
                    next_date = datetime.datetime.utcfromtimestamp(
                        int(res.get('customers[add][0][next_date]'))).strftime(
                        '%Y-%m-%d %H:%M:%S')
                    user = get_customer_user(customer_id, amo_token)
                    ...
                    cus = Customers.objects.create(amo_id=res.get('customers[add][0][id]'),
                                                   ...
                                                   next_date=next_date, created_at=data[1], updated_at=data[2],
                                                   status=data[0])
                    if data[3]:
                        cus.client = data[3]
                    if user:
    ...
    #                     user = get_customer_user(customer_custom_fields, customer_id, amo_token)
    #
    #                     cus = Customers.objects.create(amo_id=i.get('id'), name=i.get('name'),
    #                                                    price=i.get('next_price'),
    #                                                    next_date=next_date, created_at=data[1], updated_at=data[2],
    #                                                    status=data[0])
    #                     if data[3]:
    #                         cus.client = data[3]
