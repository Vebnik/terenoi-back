...
def create_lesson_notifications(lesson_status, student, date, teacher, teacher_status, lesson_id):
    if lesson_status == lessons.models.Lesson.SCHEDULED:
        if Notification.objects.filter(
            ...
            Notification.objects.create(to_user=student, lesson_date=date, type=Notification.LESSON_SCHEDULED, lesson_id=lesson_id)
        if Notification.objects.filter(
    ...
    if teacher_status:
        if Notification.objects.filter(
            ...
            Notification.objects.create(to_user=student, lesson_date=date, type=Notification.LESSON_PROGRESS,lesson_id=lesson_id)
        if Notification.objects.filter(
    ...
    if lesson_status == lessons.models.Lesson.CANCEL:
        if Notification.objects.filter(
            ...
            Notification.objects.create(to_user=student, lesson_date=date, type=Notification.LESSON_CANCEL,lesson_id=lesson_id)
        if Notification.objects.filter(
    ...
    if lesson_status == lessons.models.Lesson.REQUEST_RESCHEDULED:
        if Notification.objects.filter(
            ...
            Notification.objects.create(to_user=student, lesson_date=date, type=Notification.LESSON_REQUEST_RESCHEDULED,lesson_id=lesson_id)
        if Notification.objects.filter(
    ...
    if lesson_status == lessons.models.Lesson.RESCHEDULED:
        if Notification.objects.filter(
            ...
            Notification.objects.create(to_user=student, lesson_date=date, type=Notification.LESSON_RESCHEDULED,lesson_id=lesson_id)
        if Notification.objects.filter(
